"""Agent 2: ESTIMATOR — Claude Haiku 공종별 수량 산출 (keyword_dict 강화)"""
import json
import os
...
def _load_keyword_dict() -> dict:
    ...
    if _KEYWORD_DICT is None:
        try:
            path = Path(__file__).parent.parent / "data" / "keyword_dict.json"
            with open(path, encoding="utf-8") as f:
                data = json.load(f)
            # _source, _updated 메타 키 제거
            _KEYWORD_DICT = {k: v for k, v in data.items() if not k.startswith("_")}
        except FileNotFoundError:
            import logging
            logging.warning("keyword_dict.json not found — using empty dict")
            _KEYWORD_DICT = {}
        except Exception as e:
            import logging
            logging.warning(f"keyword_dict.json load failed: {e}")
            _KEYWORD_DICT = {}
    return _KEYWORD_DICT


def _build_keyword_context() -> str:
    """공종별 현장 키워드 매핑 컨텍스트 생성 (ESTIMATOR 프롬프트 주입용)"""
    kd = _load_keyword_dict()
    lines = ["## 현장 용어 → 공종 매핑 사전 (151개)"]
    lines.append("아래 키워드가 공사 설명에 등장하면 해당 공종으로 분류하세요:\n")
...
ESTIMATOR_SYSTEM_BASE = """당신은 인테리어/건설 공사 수량 산출 전문가입니다.
...
5. 주방 포함 여부: 주방철거+타일+싱크대+후드 (싱크대가 있으면 반드시 "주방후드" 항목을 category="주방", item="주방후드"로 추가)
...
def _get_estimator_system() -> str:
    return ESTIMATOR_SYSTEM_BASE.format(keyword_context=_build_keyword_context())
